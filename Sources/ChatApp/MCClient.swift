...
final class MCClient: ObservableObject {
    ...
    /// Height-map thực tế lấy từ Chunk Data server, dùng cho mini-map 48×48.
    @Published var mapCells: [Int64: MCMapCell] = [:]

    private var mapChunkCells: [Int64: [Int64: MCMapCell]] = [:]
    ...
    private func handlePlayPacket(id: Int32, body: Data) {
        ...
        case 0x20: // Chunk Data — protocol 340, địa hình thật của world
            handleChunkData(body)

        case 0x1D: // Unload Chunk
            handleUnloadChunk(body)

        case 0x0B: // Block Change
            handleBlockChange(body)

        case 0x10: // Multi Block Change
            handleMultiBlockChange(body)
    ...
    // MARK: - Chunk / mini-map thật

    private func chunkKey(_ chunkX: Int, _ chunkZ: Int) -> Int64 {
        (Int64(Int32(truncatingIfNeeded: chunkX)) << 32) ^ Int64(UInt32(bitPattern: Int32(truncatingIfNeeded: chunkZ)))
    }

    private func worldCellKey(_ x: Int, _ z: Int) -> Int64 {
        (Int64(Int32(truncatingIfNeeded: x)) << 32) ^ Int64(UInt32(bitPattern: Int32(truncatingIfNeeded: z)))
    }

    private func blockKind(_ blockID: Int) -> UInt8 {
        switch blockID {
        case 8, 9: return 4      // water
        case 10, 11: return 5    // lava
        case 78, 80, 174: return 6 // snow/ice
        case 18, 161: return 7   // leaves
        case 17, 162: return 8   // wood
        case 12, 24, 179: return 9 // sand
        case 13: return 10       // gravel
        case 56, 73, 74, 129: return 11 // ores
        case 49: return 12       // obsidian
        case 35, 159, 251, 252: return 13 // wool/concrete
        case 20, 95, 160: return 14 // glass
        case 45, 98, 112, 155: return 15 // bricks/building
        default: return 1        // generic terrain
        }
    }

    private func setMapChunk(_ chunkX: Int, _ chunkZ: Int, cells: [Int64: MCMapCell]) {
        let key = chunkKey(chunkX, chunkZ)
        mapChunkCells[key] = cells
        // Chỉ giữ vùng xung quanh người chơi để không làm bộ nhớ tăng mãi.
        let px = Int(floor(playerX))
        let pz = Int(floor(playerZ))
        let minX = px - 40, maxX = px + 40, minZ = pz - 40, maxZ = pz + 40
        var merged: [Int64: MCMapCell] = [:]
        for chunkCells in mapChunkCells.values {
            for (k, cell) in chunkCells where cell.x >= minX && cell.x <= maxX && cell.z >= minZ && cell.z <= maxZ {
                merged[k] = cell
            }
        }
        mapCells = merged
    ...
    /// Parse Chunk Data 1.12.x theo đúng Chunk Section format:
    /// bits-per-block -> palette -> data-array -> block light -> sky light.
    /// Bản cũ bỏ qua bits/palette nên đọc nhầm block-light như block state, làm mini-map trống/sai.
    private func handleChunkData(_ body: Data) {
        var idx = 0
        guard let cx = body.readI32BE(from: idx) else { return }; idx = cx.1
        guard let cz = body.readI32BE(from: idx) else { return }; idx = cz.1
        guard let ground = body.readU8(from: idx) else { return }; idx = ground.1
        guard let maskResult = decodeVarInt(from: body, offset: idx) else { return }
        let mask = Int(maskResult.0); idx = maskResult.1
        guard let sizeResult = decodeVarInt(from: body, offset: idx) else { return }
        let size = Int(sizeResult.0); idx = sizeResult.1
        guard size >= 0, idx + size <= body.count else { return }

        let dataEnd = idx + size
        var cells: [Int64: MCMapCell] = [:]

        // Một chunk section luôn là 4096 block. 1.12.x dùng palette gián tiếp
        // với 4..8 bits hoặc global palette trực tiếp với 9+ bits (vanilla là 13).
        for section in 0..<16 where (mask & (1 << section)) != 0 {
            guard idx < dataEnd else { return }
            let bits = Int(body[idx]); idx += 1
            let bitsPerBlock: Int
            if bits <= 4 { bitsPerBlock = 4 }
            else if bits <= 8 { bitsPerBlock = bits }
            else { bitsPerBlock = 13 }

            var palette: [Int] = []
            if bitsPerBlock <= 8 {
                guard let paletteLength = decodeVarInt(from: body, offset: idx) else { return }
                idx = paletteLength.1
                let count = Int(paletteLength.0)
                guard count >= 0, count <= 4096 else { return }
                palette.reserveCapacity(count)
                for _ in 0..<count {
                    guard let state = decodeVarInt(from: body, offset: idx) else { return }
                    idx = state.1
                    palette.append(Int(state.0))
                }
            } else {
                // Direct palette: dummy palette length, phải là VarInt 0.
                guard let dummy = decodeVarInt(from: body, offset: idx) else { return }
                idx = dummy.1
            }

            guard let longCountResult = decodeVarInt(from: body, offset: idx) else { return }
            idx = longCountResult.1
            let longCount = Int(longCountResult.0)
            guard longCount >= 0, longCount <= 1024 else { return }
            guard idx + longCount * 8 <= dataEnd else { return }

            let longStart = idx
            var longs = [UInt64]()
            longs.reserveCapacity(longCount)
            for n in 0..<longCount {
                guard let value = body.readU64BE(from: longStart + n * 8)?.0 else { return }
                longs.append(value)
            }
            idx += longCount * 8

            // 1.12.x cho phép một entry 13-bit băng qua ranh giới long.
            func paletteIndexAt(_ blockIndex: Int) -> Int {
                let bitIndex = blockIndex * bitsPerBlock
                let longIndex = bitIndex / 64
                let bitOffset = bitIndex % 64
                guard longIndex < longs.count else { return 0 }
                let maskValue: UInt64 = (UInt64(1) << UInt64(bitsPerBlock)) - 1
                var value = (longs[longIndex] >> UInt64(bitOffset)) & maskValue
                let used = 64 - bitOffset
                if used < bitsPerBlock, longIndex + 1 < longs.count {
                    value |= (longs[longIndex + 1] << UInt64(used)) & maskValue
                }
                return Int(value)
            }

            for block in 0..<4096 {
                let paletteIndex = paletteIndexAt(block)
                let state: Int
                if bitsPerBlock <= 8 {
                    guard paletteIndex < palette.count else { continue }
                    state = palette[paletteIndex]
                } else {
                    state = paletteIndex
                }

                // Global palette ID 1.12.x: (blockID << 4) | metadata.
                let blockID = state >> 4
                guard blockID != 0 else { continue }

                let localX = block & 15
                let localZ = (block >> 4) & 15
                let localY = (block >> 8) & 15
                let wx = Int(cx.0) * 16 + localX
                let wz = Int(cz.0) * 16 + localZ
                let wy = section * 16 + localY
                let key = worldCellKey(wx, wz)

                // Chỉ giữ block cao nhất ở mỗi X/Z để mini-map là height-map.
                if let old = cells[key], old.y >= wy { continue }
                cells[key] = MCMapCell(x: wx, z: wz, y: wy, kind: blockKind(blockID))
            }

            // Block light: 2048 bytes.
            guard idx + 2048 <= dataEnd else { return }
            idx += 2048

            // Overworld/End có skylight; Nether không có.
        ...
        if ground.0 != 0 {
            // Biomes của 1.12.x: 256 bytes.
            guard idx + 256 <= dataEnd else { return }
        }

        setMapChunk(Int(cx.0), Int(cz.0), cells: cells)
    }

    private func handleUnloadChunk(_ body: Data) {
        guard let cx = body.readI32BE(from: 0), let cz = body.readI32BE(from: cx.1) else { return }
        mapChunkCells.removeValue(forKey: chunkKey(Int(cx.0), Int(cz.0)))
        rebuildVisibleMap()
    }

    private func rebuildVisibleMap() {
        let px = Int(floor(playerX)), pz = Int(floor(playerZ))
        let minX = px - 40, maxX = px + 40, minZ = pz - 40, maxZ = pz + 40
        var merged: [Int64: MCMapCell] = [:]
        for chunkCells in mapChunkCells.values {
            for (k, cell) in chunkCells where cell.x >= minX && cell.x <= maxX && cell.z >= minZ && cell.z <= maxZ { merged[k] = cell }
        }
        mapCells = merged
    }

    private func handleBlockChange(_ body: Data) {
        guard let pos = body.readI64BE(from: 0), let state = decodeVarInt(from: body, offset: pos.1) else { return }
        let packed = UInt64(bitPattern: pos.0)
        func signed26(_ value: UInt64) -> Int {
            let raw = Int32(truncatingIfNeeded: value & 0x3FFFFFF)
            return Int((raw & 0x02000000) != 0 ? (raw | Int32(bitPattern: 0xFC000000)) : raw)
        }
        let x = signed26(packed >> 38)
        let z = signed26((packed >> 12) & 0x3FFFFFF)
        let y = Int(packed & 0xFFF)
        let blockID = Int(state.0) >> 4
        let key = worldCellKey(x, z)
        if blockID == 0 { mapCells.removeValue(forKey: key) } else { mapCells[key] = MCMapCell(x: x, z: z, y: y, kind: blockKind(blockID)) }
    }

    private func handleMultiBlockChange(_ body: Data) {
        var idx = 0
        guard let chunk = body.readI32BE(from: idx) else { return }; idx = chunk.1
        guard let chunkZ = body.readI32BE(from: idx) else { return }; idx = chunkZ.1
        guard let count = body.readU16BE(from: idx) else { return }; idx = count.1
        for _ in 0..<Int(count.0) {
            guard let record = body.readU16BE(from: idx) else { return }; idx = record.1
            guard let state = decodeVarInt(from: body, offset: idx) else { return }; idx = state.1
            let localX = Int((record.0 >> 12) & 0xF)
            let localZ = Int((record.0 >> 8) & 0xF)
            let y = Int(record.0 & 0xFF)
            let x = Int(chunk.0) * 16 + localX
            let z = Int(chunkZ.0) * 16 + localZ
            let key = worldCellKey(x, z)
            let blockID = Int(state.0) >> 4
            if blockID == 0 { mapCells.removeValue(forKey: key) }
            else { mapCells[key] = MCMapCell(x: x, z: z, y: y, kind: blockKind(blockID)) }
        }
    }
...
private func zlibInflate(_ data: Data, decompressedSize: Int) -> Data? {
    guard decompressedSize > 0, !data.isEmpty else { return nil }
    // Compression.COMPRESSION_ZLIB nhận trực tiếp zlib stream (đã gồm header + checksum).
    // Không được tự cắt header/trailer vì sẽ làm decoder đọc sai dữ liệu.
    ...
    let resultSize: Int = result.withUnsafeMutableBytes { dst -> Int in
        data.withUnsafeBytes { src -> Int in
            ...
            return compression_decode_buffer(dstPtr, decompressedSize, srcPtr, data.count, nil, COMPRESSION_ZLIB)
        }
    }
...
private func zlibDeflate(_ data: Data) -> Data {
    // COMPRESSION_ZLIB đã tạo ra zlib stream hoàn chỉnh.
    // Không thêm header 78 9C hoặc Adler-32 lần nữa.
