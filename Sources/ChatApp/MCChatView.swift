        ...
        VStack(spacing: 0) {
            VStack(spacing: 8) {
                HStack(alignment: .top, spacing: 12) {
                    MiniMapView(x: client.playerX, z: client.playerZ, cells: client.mapCells, trail: client.movementTrail)
                        .frame(width: 184, height: 184)
                    VStack(alignment: .leading, spacing: 6) {
                        Text(String(format: "X=%.2f\nY=%.2f\nZ=%.2f", client.playerX, client.playerY, client.playerZ))
                            .font(.system(.caption, design: .monospaced))
                        Text("\(client.currentDimension) · \(client.isOnGroundLabel)")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                        Text(client.mapCells.isEmpty ? "Đang tải địa hình..." : "Mini-map: địa hình server")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
...
private struct MiniMapView: View {
    let x: Double
    let z: Double
    let cells: [Int64: MCMapCell]
    let trail: [(x: Double, z: Double)]

    private func terrainColor(_ kind: UInt8) -> Color {
        switch kind {
        case 4: return .blue.opacity(0.72)
        case 5: return .red.opacity(0.72)
        case 6: return .white.opacity(0.82)
        case 7: return .green.opacity(0.75)
        case 8: return .brown.opacity(0.78)
        case 9: return .yellow.opacity(0.78)
        case 10: return .gray.opacity(0.78)
        case 11: return .cyan.opacity(0.78)
        case 12: return .purple.opacity(0.78)
        case 13: return .pink.opacity(0.72)
        case 14: return .mint.opacity(0.72)
        case 15: return .orange.opacity(0.78)
        default: return .green.opacity(0.52)
        }
    }
    ...
    var body: some View {
        GeometryReader { geo in
            Canvas { context, size in
                let cell = min(size.width, size.height) / 48.0
                let center = CGPoint(x: size.width / 2, y: size.height / 2)
                let minX = Int(floor(x)) - 24
                let maxX = minX + 47
                let minZ = Int(floor(z)) - 24
                let maxZ = minZ + 47

                // Nền và lưới 48×48.
                context.fill(Path(CGRect(origin: .zero, size: size)), with: .color(.black.opacity(0.18)))
                for ix in 0...48 {
                    var p = Path()
                    p.move(to: CGPoint(x: CGFloat(ix) * cell, y: 0))
                    p.addLine(to: CGPoint(x: CGFloat(ix) * cell, y: size.height))
                    context.stroke(p, with: .color(.secondary.opacity(0.12)), lineWidth: 0.35)
                }
                for iz in 0...48 {
                    var p = Path()
                    p.move(to: CGPoint(x: 0, y: CGFloat(iz) * cell))
                    p.addLine(to: CGPoint(x: size.width, y: CGFloat(iz) * cell))
                    context.stroke(p, with: .color(.secondary.opacity(0.12)), lineWidth: 0.35)
                }

                for cellData in cells.values {
                    guard cellData.x >= minX && cellData.x <= maxX && cellData.z >= minZ && cellData.z <= maxZ else { continue }
                    let px = center.x + CGFloat(Double(cellData.x) - x) * cell
                    let py = center.y + CGFloat(Double(cellData.z) - z) * cell
                    let rect = CGRect(x: px - cell / 2, y: py - cell / 2, width: cell + 0.8, height: cell + 0.8)
                    context.fill(Path(rect), with: .color(terrainColor(cellData.kind)))
                }
                ...
                // Người chơi + hướng nhìn.
                var player = Path()
                player.move(to: CGPoint(x: center.x, y: center.y - 6))
                player.addLine(to: CGPoint(x: center.x + 6, y: center.y + 6))
                player.addLine(to: CGPoint(x: center.x - 6, y: center.y + 6))
                player.closeSubpath()
