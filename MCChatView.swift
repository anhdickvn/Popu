    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                HStack(spacing: 16) {
                    // Nút chat: quay lại màn hình chat từ bảng điều khiển WASD/Jump.
                    Button { showControls = false } label: {
                        Image(systemName: "message.fill")
                    }
                    .opacity(showControls ? 1 : 0.65)

                    // Nút điều khiển riêng: WASD + Jump không còn nằm trong khu vực chat.
